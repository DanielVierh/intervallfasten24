...
const btn_start_stop_longFasting = document.getElementById(
  "btn_start_stop_longFasting"
);
...
const lbl_longtimeFastingTime = document.getElementById(
  "lbl_longtimeFastingTime"
);
...
const fastingSince = document.getElementById(
  "fastingSince"
) as HTMLInputElement;
...
let is_longtime_fasting: boolean = false;
...
function checkFastingStatus() {
  ...
  if (diffToFastingInSeconds < intervalEventObject.eatTime * 60 * 60) {
    ...
    outputFrom!.innerHTML = `${addZero(fastingStartTimeMinusEatTime)}:${addZero(
      fastingStartMinute
    )}`;
    ...
    outputTo!.innerHTML = `${addZero(fastingStartTimeMinusEatTime)}:${addZero(
      fastingStartMinute
    )}`;
...
function minutesDiff(dateTimeValue2: Date, dateTimeValue1: Date) {
  ...
  const pure_hours = Math.floor(rawMinuteTime / 60);

  const time = `Tage: ${add_zero(days)} \n Stunden: ${add_zero(
    hours
  )} \n Minuten: ${add_zero(minutes)} <br> Stunden Gesamt: ${pure_hours}`;
  return time;
}
...
function load_from_LocalStorage() {
  if (localStorage.getItem("stored_IntervallObj") !== null) {
    ...
    intervalEventObject = JSON.parse(
      localStorage.getItem("stored_IntervallObj")
    );
    fastingChangeButton!.innerText = `${intervalEventObject.fastingTime}:${intervalEventObject.eatTime}`;
    try {
...
btnReset?.addEventListener("click", () => {
  if (intervalEventObject.water > 0) {
    const confirm = window.confirm(
      `Soll die getrunkene Menge von ${intervalEventObject.water.toFixed(
        2
      )} L wirklich zurückgesetzt werden?`
    );
    if (confirm) {
      intervalEventObject.water = 0;
...
btn_start_stop_longFasting?.addEventListener("click", () => {
  if (is_longtime_fasting === false) {
    ...
    const confirm_longtimeFasting = window.confirm(
      "Soll ein längeres Fasten gestartet werden?"
    );
    ...
    if (confirm_longtimeFasting) {
      ...
      is_longtime_fasting = true;
      btn_start_stop_longFasting.innerHTML = "Stoppe Fasten";
      intervalEventObject.longTimeFastingStart = String(
        longfasting_start_stamp
      );
      save_into_LocalStorage();
      modal_longtimeFasting?.classList.add("active");
    }
    ...
    const confirm_longtimeFasting_Stop = window.confirm(
      "Soll das Fasten gestoppt werden?"
    );
